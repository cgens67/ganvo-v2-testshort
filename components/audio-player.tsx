...
import {
  ...
  ArrowLeft, Palette, LayoutTemplate, CornerUpRight, AudioLines, Type, ShieldAlert, Speaker
} from "lucide-react"
...
export function AudioPlayer() {
  ...
  const[audioQuality, setAudioQuality] = useState<'High' | 'Normal' | 'Data Saver'>('High')
  ...
  const getLyricTextClass = () => {
    ...
    return "text-xl md:text-2xl" // Normal
  }
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/operation-not-allowed') setAuthError("Email/Password login is not enabled in Firebase Console.")
      else if (error.code === 'auth/invalid-credential') setAuthError("Incorrect email or password.")
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/operation-not-allowed') setAuthError("Google login is not enabled in Firebase Console.")
      else setAuthError(error.message.replace("Firebase: ", ""))
    }
  }
  ...
  useEffect(() => {
    ...
    const loadStream = async () => {
      ...
      } catch (error) { setLoadError("Network error. Please try again.") } finally { setIsLoading(false) }
    }
    loadStream()
  }, [currentSong?.videoId])
  ...
  const handleLoadedData = () => {
    applyAudioEffects()
    if (isPlaying) audioRef.current?.play().catch(() => setLoadError("Playback failed."))
  }
  ...
  return (
    <div className={cn("flex h-screen flex-col overflow-hidden bg-background transition-colors duration-500 font-sans relative z-0", dynamicTheme && playerBgStyle === 'Gradient' && dominantColor ? "animate-gradient" : "")} 
      ...
      <header className="elevation-1 z-40 flex h-16 flex-shrink-0 items-center justify-between px-3 md:px-6 transition-all duration-500 ease-[cubic-bezier(0.2,0,0,1)] relative bg-background/80 backdrop-blur-xl border-b border-border/40 gap-2">
        <div className={cn(
              ...
              {isSearching && searchResults.length === 0 ? (
                ...
                <div className="flex items-center justify-between p-3 border-b bg-muted/20">
                  ...
                  <Select value={searchSort} onValueChange={(v: any) => setSearchSort(v)}>
                    <SelectTrigger className="h-7 text-xs w-[120px] rounded-full border-none bg-muted font-bold text-foreground">
            ...
            <DropdownMenuContent align="end" className="w-56 rounded-2xl animate-in fade-in zoom-in-95 duration-300 ease-out p-2 shadow-xl border-border/50 z-[250]">
              {user ? (
                ...
                <div className="px-2 py-2 mb-1">
                  ...
                  <Button onClick={() => setShowAuthDialog(true)} className="w-full justify-start rounded-xl font-bold transition-all active:scale-[0.98] bg-primary text-primary-foreground hover:bg-primary/90" size="sm">Sign In / Sign Up</Button>
                </div>
              )}
        ...
        <div className="flex flex-1 flex-col overflow-y-auto min-h-0 z-10 pb-40 lg:pb-0 transition-all duration-500 ease-out">
          ...
          {activeTab === 'explore' ? (
                          ...
                          {exploreData.songs.slice(0, 9).map((song, idx) => (
                            <div key={idx} className="group flex items-center gap-4 p-3 rounded-2xl hover:bg-muted transition-colors duration-300 ease-out bg-card shadow-sm border">
                              ...
                              {playlists.length > 0 && (
                                <DropdownMenu>
                                  <DropdownMenuTrigger asChild>
                                    <Button variant="ghost" size="icon" className="h-10 w-10 text-muted-foreground hover:text-primary opacity-0 group-hover:opacity-100 transition-opacity"><ListPlus className="h-5 w-5 text-current" /></Button>
                                  </DropdownMenuTrigger>
                              ...
                              <Button variant="secondary" size="icon" onClick={() => addToQueueAndPlay(song)} className="rounded-full opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-300 active:scale-95 text-foreground h-10 w-10 bg-secondary hover:bg-secondary/80">
                     ...
                     <div>
                       ...
                       <div className="flex overflow-x-auto pb-4 gap-6 snap-x no-scrollbar px-1 py-4 -my-4 -mx-4 px-4">
                          {exploreData.albums.map((album, idx) => (
                            <div key={idx} onClick={() => loadAlbumView(album.browseId)} className="flex flex-col gap-3 w-40 sm:w-48 shrink-0 group cursor-pointer snap-start">
                    ...
                    <div className="space-y-1 md:space-y-2 mb-12 bg-card p-4 rounded-[2rem] border shadow-sm">
                      {currentArtistData.topSongs?.map((song: any, idx: number) => (
                        <div key={song.videoId} className="group flex items-center gap-3 md:gap-4 p-2 md:p-3 rounded-2xl hover:bg-muted transition-colors duration-300 ease-out">
                          ...
                          <Button variant="secondary" size="sm" onClick={() => addToQueueAndPlay(song)} className="rounded-full font-bold opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-300 active:scale-95 shadow-sm text-foreground bg-secondary hover:bg-secondary/80">
                      ...
                      <div className="mb-8">
                        ...
                        <div className="flex overflow-x-auto pb-4 gap-4 snap-x no-scrollbar py-4 -my-4 -mx-4 px-4">
                          {currentArtistData.singles.map((single: any, idx: number) => (
                            <div key={idx} onClick={() => loadAlbumView(single.browseId)} className="flex flex-col gap-3 w-40 sm:w-48 shrink-0 group cursor-pointer snap-start">
                              <div className="overflow-hidden rounded-2xl shadow-md transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:scale-105 group-active:scale-95 aspect-square relative">
            ...
            <div className="p-4 md:p-10 max-w-6xl mx-auto w-full animate-in fade-in slide-in-from-right-8 duration-700 ease-out">
                ...
                {isAlbumLoading ? (
                  ...
                  <div className="animate-in fade-in duration-500">
                    <div className="flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-8 mb-12 p-4 md:p-8">
                      <img src={currentAlbumData.thumbnails?.[currentAlbumData.thumbnails.length-1]?.url || "/placeholder.svg"} alt={currentAlbumData.name} className="w-48 h-48 md:w-64 md:h-64 rounded-2xl object-cover shadow-2xl" />
                      <div className="text-center md:text-left flex-1 flex flex-col justify-end">
                        ...
                        <div className="flex gap-3 justify-center md:justify-start">
                          <Button onClick={() => addToQueueAndPlay(currentAlbumData.songs[0])} className="rounded-full font-bold text-base px-8 h-12 shadow-lg hover:scale-105 active:scale-95 transition-all bg-primary text-primary-foreground">
                    ...
                    <div className="space-y-1 mb-12 bg-card p-2 md:p-4 rounded-[2rem] border shadow-sm">
                      {currentAlbumData.songs?.map((song: any, idx: number) => (
                        <div key={song.videoId} className="group flex items-center gap-3 md:gap-4 p-2 md:p-3 rounded-xl hover:bg-muted transition-colors duration-300 ease-out">
                          ...
                          <Button variant="secondary" size="icon" onClick={() => addToQueueAndPlay(song)} className="rounded-full font-bold opacity-100 md:opacity-0 group-hover:opacity-100 transition-all duration-300 active:scale-95 shadow-sm text-foreground bg-secondary hover:bg-secondary/80">
                            <Play className="h-5 w-5 fill-current text-current translate-x-[1px]"/>
                          </Button>
                          <Button variant="ghost" size="icon" onClick={() => toggleLike(song)} className={cn("text-muted-foreground hover:text-[var(--google-red)] transition-all", likedSongs.has(song.videoId) && "text-[var(--google-red)]")}>
                            <Heart className={cn("h-5 w-5", likedSongs.has(song.videoId) && "fill-current text-current")} />
             ...
             /* Playlist View */
             <div className="p-4 md:p-10 max-w-6xl mx-auto w-full animate-in fade-in slide-in-from-right-8 duration-700 ease-out">
                <Button variant="ghost" onClick={() => setActiveTab('library')} className="mb-6 -ml-2 md:-ml-4 gap-2 font-bold text-muted-foreground hover:text-foreground transition-all">
                ...
                <div className="animate-in fade-in duration-500">
                  <div className="flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-8 mb-12 p-4 md:p-8">
                    ...
                    <div className="text-center md:text-left flex-1 flex flex-col justify-end">
                      ...
                      <div className="flex gap-3 justify-center md:justify-start">
                        <Button onClick={() => {if(currentPlaylistView.songs.length>0) addToQueueAndPlay(currentPlaylistView.songs[0])}} disabled={currentPlaylistView.songs.length===0} className="rounded-full font-bold text-base px-8 h-12 shadow-lg hover:scale-105 active:scale-95 transition-all bg-primary text-primary-foreground">
          ...
          <div className="flex-1 overflow-y-auto min-h-0 overscroll-contain transition-all duration-500">
            {activeTab === 'explore' || activeTab === 'artist' || activeTab === 'player' || activeTab === 'album' || activeTab === 'playlistView' ? (
              ...
              <div className="h-full flex flex-col relative">
                <div className="flex-1 overflow-y-auto no-scrollbar scroll-smooth lyrics-scroll-container pb-32">
                  {lyrics?.syncedLyrics ? (
                    <div className="space-y-5 p-6">
                      {lyrics.syncedLyrics.map((line, index) => (
                        <p key={index} onClick={() => { if (audioRef.current) audioRef.current.currentTime = line.time }} className={cn("lyric-line transition-all duration-500 ease-out cursor-pointer rounded-2xl px-4 py-3 font-bold leading-relaxed text-center", getLyricTextClass(), index === currentLyricIndex ? "lyric-active-line scale-[1.05] bg-primary/10 text-primary shadow-sm" : index < currentLyricIndex ? "text-muted-foreground/30 scale-95" : "text-muted-foreground/70 hover:bg-muted hover:text-foreground scale-95")}>
                          {line.text}
                        </p>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center py-24 text-center h-full"><Mic2 className="h-10 w-10 text-muted-foreground/40 mb-6" /><p className="font-extrabold text-xl mb-2 text-foreground">Couldn't find timed lyrics</p><p className="text-sm font-medium text-muted-foreground px-4 mt-2">Try switching providers in Appearance Settings.</p></div>
                  ) : (
               ...
               <div className="p-4 space-y-6 pb-32 animate-in slide-in-from-bottom-8 duration-700 ease-out">
                  ...
                  <div>
                    <div className="mb-4 px-2 flex items-center justify-between">
                      ...
                      <Button variant="secondary" size="sm" onClick={() => setShowPlaylistDialog(true)} className="rounded-xl font-bold text-xs h-8 text-foreground bg-secondary hover:bg-secondary/80">New</Button>
                    </div>
                    {playlists.length > 0 ? (
      ...
      <div 
        className={cn(
          "fixed inset-x-0 bottom-0 z-[200] bg-background flex flex-col transition-transform duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] lg:hidden rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.3)]",
          ...
          {mobilePlayerTab === 'player' && currentSong && (
            <div className="flex flex-col items-center h-full animate-in fade-in zoom-in-95 duration-500 flex-1 justify-center">
              ...
              <div className="flex w-full items-center justify-between gap-3 px-2">
                ...
                <Button variant="secondary" size="icon" onClick={() => setShowEffectsDialog(true)} className="h-12 w-12 flex-shrink-0 rounded-2xl transition-transform duration-300 hover:scale-110 active:scale-90 text-foreground bg-secondary hover:bg-secondary/80 shadow-sm flex items-center justify-center">
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar scroll-smooth lyrics-scroll-container pb-32">
                  {lyrics?.syncedLyrics ? (
                    <div className="space-y-6 py-10">
                      {lyrics.syncedLyrics.map((line, index) => (
                        <p key={index} onClick={() => { if (audioRef.current) audioRef.current.currentTime = line.time }} className={cn("lyric-line transition-all duration-500 ease-out cursor-pointer rounded-3xl px-6 py-4 font-extrabold leading-tight text-center", getLyricTextClass(), index === currentLyricIndex ? "lyric-active-line scale-[1.05] bg-primary/10 text-primary shadow-sm" : index < currentLyricIndex ? "text-muted-foreground/30 scale-95" : "text-muted-foreground/70 hover:bg-muted hover:text-foreground scale-95")}>
                          {line.text}
                        </p>
                      ))}
                    </div>
      ...
      <Dialog open={showEffectsDialog} onOpenChange={setShowEffectsDialog}>
        <DialogContent className="rounded-[2rem] sm:max-w-md p-8 border-0 shadow-2xl animate-in zoom-in-95 duration-500 ease-out outline-none bg-background !z-[400]">
          ...
          <div className="space-y-8 mt-6">
            ...
            <Button onClick={() => {setPlaybackRate(1); setPreservesPitch(true)}} variant="secondary" className="w-full h-12 rounded-xl font-bold text-foreground bg-secondary hover:bg-secondary/80">Reset to Default</Button>
          </div>
        </DialogContent>
      </Dialog>
             ...
             <div className="px-4 py-4 space-y-1">
               ...
               <div className="flex items-center justify-between p-3 bg-transparent rounded-2xl cursor-pointer hover:bg-muted/50 transition-colors">
                 ...
                 <Select value={playerBgStyle} onValueChange={(v: any) => setPlayerBgStyle(v)}>
                    <SelectTrigger className="w-[120px] rounded-xl font-bold bg-muted border-none text-foreground text-xs h-9">
               ...
               <div className="flex items-center justify-between p-3 mt-2 bg-transparent rounded-2xl cursor-pointer hover:bg-muted/50 transition-colors">
                 <div className="flex items-center gap-4">
                   ...
                   <div className="flex flex-col">
                     <span className="font-bold text-base text-foreground">Lyrics provider</span>
                   </div>
                 </div>
               ...
               <div className="flex items-center justify-between p-3 mt-2 bg-transparent rounded-2xl cursor-pointer hover:bg-muted/50 transition-colors">
                 <div className="flex items-center gap-4">
                   ...
                   <div className="flex flex-col">
                     ...
                     <span className="text-sm font-normal text-muted-foreground">{audioQuality}</span>
                   </div>
                 </div>
                 <Select value={audioQuality} onValueChange={(v: any) => setAudioQuality(v)}>
                    ...
                    <SelectContent className="rounded-2xl z-[500]">
                      ...
                      <SelectItem value="Normal" className="font-bold py-2 text-xs">Normal</SelectItem>
                      <SelectItem value="Data Saver" className="font-bold py-2 text-xs">Data Saver</SelectItem>
                    </SelectContent>
                  </Select>
               </div>
               ...
               <div className="flex items-center justify-between p-3 mt-2 bg-transparent rounded-2xl cursor-pointer hover:bg-muted/50 transition-colors">
                 <div className="flex items-center gap-4">
                   <div className="p-2 bg-muted/80 rounded-full text-foreground"><ShieldAlert className="w-5 h-5 text-current"/></div>
                   <span className="font-bold text-base text-foreground">Show explicit content</span>
                 </div>
                 <Switch checked={showExplicit} onCheckedChange={setShowExplicit} className="data-[state=checked]:bg-[var(--google-blue)]" />
               </div>

               <div className="flex items-center justify-between p-3 mt-2 bg-transparent rounded-2xl cursor-pointer hover:bg-muted/50 transition-colors">
                 <div className="flex items-center gap-4">
                   <div className="p-2 bg-muted/80 rounded-full text-foreground"><Volume2 className="w-5 h-5 text-current"/></div>
                   <span className="font-bold text-base text-foreground">Normalize volume</span>
                 </div>
                 <Switch checked={normalizeVolume} onCheckedChange={setNormalizeVolume} className="data-[state=checked]:bg-[var(--google-blue)]" />
               </div>
